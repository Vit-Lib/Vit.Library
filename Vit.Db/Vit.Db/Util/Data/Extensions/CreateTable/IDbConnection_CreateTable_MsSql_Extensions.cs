...
namespace Vit.Extensions
{
    public static partial class IDbConnection_CreateTable_MsSql_Extensions
    {
        ...
        /// <param name="dt"></param>
        public static void MsSql_CreateTable(this IDbConnection conn, DataTable dt)
        {
            if (null == dt || dt.Columns.Count == 0) return;

            //创建表结构的SQL语句

            StringBuilder sql = new StringBuilder("Create Table ").Append(conn.MsSql_Quote(dt.TableName)).Append(" (");
            ...
            foreach (DataColumn dc in dt.Columns)
            {
                ...
                sql.Append(conn.MsSql_Quote(columnName)).Append(" ").Append(ClrType_To_DbType(type)).Append(",");
            }
            sql.Length--;
            sql.Append(")");
        ...
        public static void MsSql_CreateTable(this IDbConnection conn, IDataReader dr, string tableName)
        {

            if (null == dr || dr.FieldCount == 0) return;

            //创建表结构的SQL语句
            StringBuilder sql = new StringBuilder("Create Table ").Append(conn.MsSql_Quote(tableName)).Append(" (");
